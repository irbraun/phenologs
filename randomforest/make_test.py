...
# Generates slurm submit scripts for learn_test.R
...
ontology = sys.argv[6]
k = sys.argv[7]
num_features = sys.argv[8]
num_testing_files = sys.argv[9]
testpart_lower = sys.argv[10]
testpart_upper = sys.argv[11] #inclusive
split_postfix = sys.argv[12]
rf_filename = base_dir+sys.argv[13]
...
f.write("\n")
f.write("\n")
f.write("Rscript %s %s %s %s %s %s "%(r_script_name, ontology, k, num_features, output_root, num_testing_files))
for part in range(int(testpart_lower),int(testpart_upper)+1):
	f.write("%s%s.%d.csv "%(data_path,data_name,part))
f.write("%s %s"%(split_postfix, rf_filename))
f.close()