...
# 4. testing_data
# 5. split_postfix
# 6. classprobs_filename
# 7. classprobs_identifier
...
classprobs_filename <- args[(5+num_testing_files)]
classprobs_id <- args[(6+num_testing_files)]


# Read in the data.
results <- read_list(testing_filenames)
classprobs <- read.csv(file=classprobs_filename, sep=",", stringsAsFactors=FALSE)
...
results <- left_join(results, classprobs, by=c("term","chunk"))
...
filenames.pr <- paste(output_root, "pr_", split_postfix, "_", classprobs_id, ".csv", sep="")
