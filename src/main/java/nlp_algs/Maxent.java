...
public class Maxent implements SoftClassifier<int[]>, Serializable {
    ...
    private static double dot(int[] x, double[] w) {
        ...
        // not augmented with 1, the extra value at the end of w
        // so only use the first n-1 values in w which equals the number of elements in x.
        double dot = 0.00;
    ...
    public double predict(int[] x, int y) {
        return predict(x, null, y);
    }
    
    
    
    /**
     * Issues with training for the max ent classifier. When using the tolerance of 1E-4 and the 
     * minimum of 100 training iterations then only the first two features are actually adjusted
     * from having weights of zero at all. So when testing for every single class those features
     * will only 
     * @param x
     * @param posteriori
     * @param y
     * @return 
     */
    public double predict(int[] x, double[] posteriori, int y) {
        if (posteriori != null && posteriori.length != k) {
            throw new IllegalArgumentException(String.format("Invalid posteriori vector size: %d, expected: %d", posteriori.length, k));
        }
        ...
        if (w != null) {
            ...
            logger.info(String.format("w for %s has length %s and %s non-zero elements", y, subW.length, ctr));
            // end verifying stuff.
    ...
    public double[] getWeights(int y){
        return W[y];
    }
